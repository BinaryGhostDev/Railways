...
def search_booked_ticket_by_pnr(connection, current_user):
    try:
...
def show_all_booked_tickets(connection, current_user):
    try:
        cursor = connection.cursor(dictionary=True)
        ...
        cursor.execute("""
            ...
            FROM bookings
            WHERE booked_user_id = %s AND status = %s
        """, (current_user['id'], 'BOOKED'))
...
def show_all_tickets(connection, current_user):
    try:
        cursor = connection.cursor(dictionary=True)
        ...
        if not booked_tickets:
            ...
            # Print the table after the loop
            print("")
            success_message(" Your All Tickets: ")
            print("")
            print(table)
...
def cancel_booked_ticket(connection, current_user):
    try:
            ...
            if booked_ticket['status'] == 'CANCELED':
                ...
                if current_date <= journey_date <= current_date + timedelta(days=1):
                    ...
                    if current_time >= departure_time and current_time <= arrival_time:
                        ...
                        connection.commit()
                        print("")
                        error_message ("This Train is Departured If you want to cancel call us on 129")
